...
#define LOWTIME 8 // number of 48MHz cycles to be low for 0.35uS // originally 15
#define HIGHTIME 38 // number of 48MHz cycles to be high for 1.65uS // originally 65
...
void ws2812b_setColor(wsColor * c) {
    int i = 0; int j = 0; // for loops
    int numBits = 2 * 3 * 8 * 8; // the number of high/low bits to store, 2 per color bit
    ...
    for (i = 0; i < 8; i++) {
        ...
        for (j = 7; j >= 0; j--) {
            // if the bit is a 1
            if (c[i].r>>j&0b00000001 == 0b000000001) {
                // the high is longer
        ...
        for (j = 7; j >= 0; j--) {
            // if the bit is a 1
            if (c[i].g>>j&0b00000001 == 0b000000001) {
                // the high is longer
                delay_times[nB] = delay_times[nB - 1] + HIGHTIME;
                nB++;
                delay_times[nB] = delay_times[nB - 1] + LOWTIME;
        ...
        for (j = 7; j >= 0; j--) {
            // if the bit is a 1
            if (c[i].b>>j&0b00000001 == 0b000000001) {
                // the high is longer
                delay_times[nB] = delay_times[nB - 1] + HIGHTIME;
                nB++;
                delay_times[nB] = delay_times[nB - 1] + LOWTIME;
